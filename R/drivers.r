...
makeA <- function(ped,which,inbreeding=NULL)
  {
    if(!is.data.frame(ped))stop("ped should be data.frame")
    ...
    if(!is.null(inbreeding) & length(inbreeding)!=nrow(ped))
       stop("length inbreeding should match number of rows in pedigree")
    ...
    if (is.null(inbreeding)) {
       ...
       # run getAi version if inbreeding is supplied
       inbreeding <- as.numeric(inbreeding)
       res <- .C("getAi",ind = as.integer(id),dam = as.integer(dam),sire = as.integer(sire),
              n = as.integer(n),which = as.integer(which), inbreeding=as.numeric(inbreeding))
       return(TRUE)
    }
  }
